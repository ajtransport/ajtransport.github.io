class ThemeCustomize {
    ...
    registerEvents() {
        ...
        jQuery(document).on("click", ".tbdemo-device-mobile", function () {
            ...
            if( self.active_color != null && self.active_color != "" ) {
                let kit = self.colors.filter((value) => value['id'] === self.active_color)[0]['kit'];
        ...
        jQuery(document).on('click', 'header button, .twbb-top-bar-icon-parent, .twbb-theme-customize-close', function() {
            self.close_customization();
        });

        jQuery(document).on('click', '.twbb-theme-customize-button-editor', function() {
            self.save_customization(this);
        });
    }
    ...
    setActiveTheme() {
        ...
        if(!twbb_theme_color){
            twbb_theme_color = twbb_theme_customize.active_color;
        }


        if(!twbb_theme_color){
            twbb_theme_color = twbb_theme_customize.active_font;
        }
    ...
    changeThemeStyle( param, type ) {
        ...
        if( type == 'font' ) {
            ...
            this.changeFonts(jQuery('body'), font);
            if( iframeBody.length ) {
                this.changeFonts(iframeBody, font);
            }
        } else if( type == 'color' ) {
            if( self.from_demo ) {
                document.cookie = "twbb_theme_color=" + this.active_color + "; path=/";
            }
            let kit = this.colors.filter((value) => value['id'] === this.active_color)[0]['kit']
            this.changeColors(jQuery('body'), kit);
            if(iframeBody.length) {
